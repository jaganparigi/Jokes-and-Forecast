...
import { get5dayForecast, getCityData } from "../Store/Slices/WeatherSlice";
...
const Weather = () => {
  ...
  //   allLoadings.some((state) => state) || loadings;
  ...
  const filterForecastByFirstObTime = (forecastData) => {
    if (!forecastData) {
      return [];
    }
  ...
  const filteredForecast = filterForecastByFirstObTime(forecastData?.list);
